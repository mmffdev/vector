...
function ReportItem({ report }: { report: MemoryReport }) {
  ...
  const hasOverrideName = !!report.scopeName && report.scopeName !== SCOPE_LABELS[report.scope];
  const titleName = hasOverrideName
    ? report.scopeName
    : (SCOPE_LABELS[report.scope] ?? report.scope);
  const badge = hasOverrideName
    ? (report.scopeName?.[0]?.toUpperCase() ?? "•")
    : report.scope;

  const header = (
    <span className="dui-meta">
      <span className="dui-meta__id">{badge}</span>
      <span className="dui-meta__title">{titleName} scan</span>
