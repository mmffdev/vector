...
import type { CodeMeta } from "@/app/api/dev/code/route";
import { DevAccordion, DevAccordionItem } from "@dev/components/DevAccordion";
import Panel from "@/app/components/Panel";

const STORAGE_KEY = "dev-setup-open-code";
...
function CodeItem({ meta, isOpen, onToggle }: { meta: CodeMeta; isOpen: boolean; onToggle: (open: boolean) => void }) {
  ...
  async function loadContent() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/dev/code?id=${encodeURIComponent(meta.id)}`);
...
export default function DevCodePanel() {
  const [reports, setReports] = useState<CodeMeta[]>([]);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(ids)) setOpenIds(new Set(ids));
      } catch { /* ignore */ }
    }
  }, []);
  ...
  async function load() {
    ...
    try {
      ...
      setError(e?.message ?? "Failed to load code reports.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Panel name="dev_code" title="Code">
      <div className="dui-page">
        <header className="dui-page__header">
          <div>
            <h1 className="dui-page__title">Code</h1>
            <p className="dui-page__subtitle">
              Codebase quality checks. Run <code>&lt;code&gt; -d</code> in the Claude CLI to trace a file&apos;s
              import/dependency graph and flag boundary-crossing imports. Each run saves a JSON file in <code>dev/code/</code>.
            </p>
          </div>
          <button
            onClick={load}
        ...
        {!loading && reports.length === 0 && !error && (
          <div className="dui-empty">
            No code reports yet. Run <code>&lt;code&gt; -d</code> in the Claude Code CLI to generate one.
          </div>
        )}
            ...
            <DevAccordion>
              {visible.map(r => (
                <CodeItem
                  ...
                  onToggle={open => toggleOpen(r.id, open)}
                />
              ))}
            </DevAccordion>
