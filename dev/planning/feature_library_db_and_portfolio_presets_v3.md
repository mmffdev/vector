...
**Subscription-archive saga** — Postgres-backed queue (decided over RabbitMQ; see Section 17.1 rationale).

Queue table in `mmff_vector` so enqueue is atomic with the archive transaction:

```sql
CREATE TABLE pending_library_cleanup_jobs (
    id              UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    job_kind        TEXT        NOT NULL CHECK (job_kind IN ('archive_subscription')),
    payload         JSONB       NOT NULL,                       -- {"subscription_id": "..."}
...
CREATE INDEX idx_cleanup_jobs_runnable
  ON pending_library_cleanup_jobs(next_attempt_at)
  WHERE dead_letter = FALSE AND completed_at IS NULL;
```

**Enqueue** (in the same transaction as the archive):
```sql
BEGIN;
UPDATE subscriptions SET archived_at = NOW() WHERE id = $sub;
INSERT INTO pending_library_cleanup_jobs (job_kind, payload)
  VALUES ('archive_subscription', jsonb_build_object('subscription_id', $sub));
COMMIT;
```

**Worker** (`backend/internal/library/cleanup_worker.go`, in-process Go ticker, 30s interval):
1. `SELECT * FROM pending_library_cleanup_jobs WHERE dead_letter=FALSE AND completed_at IS NULL AND next_attempt_at <= NOW() FOR UPDATE SKIP LOCKED LIMIT 10`.
2. For each job: open library tx, run cleanup steps, commit.
3. On success: `UPDATE … SET completed_at = NOW()`.
4. On failure: `UPDATE … SET attempts = attempts + 1, last_error = $err, next_attempt_at = NOW() + (2^attempts) * interval '30 seconds'`. Once `attempts >= max_attempts`, flip `dead_letter = TRUE`.

**Cleanup steps** (per job):
- `UPDATE portfolio_models SET archived_at = NOW() WHERE owner_subscription_id = $sub AND archived_at IS NULL`.
- `UPDATE portfolio_model_shares SET revoked_at = NOW() WHERE (grantee_subscription_id = $sub OR model_id IN (SELECT id FROM portfolio_models WHERE owner_subscription_id = $sub)) AND revoked_at IS NULL`.
...
**Operator surface** — `SELECT * FROM pending_library_cleanup_jobs WHERE dead_letter = TRUE` answers "what's stuck?" Surface this in `<dev>` admin tooling.
