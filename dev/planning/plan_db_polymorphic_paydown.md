...
- [x] **2.1** Create `backend/internal/entityrefs/service.go` (or co-locate per relationship if the team prefers). Implement `Insert(ctx, tx, kind, id, callerTenant, …) error` and `DeleteByParent(ctx, tx, kind, id) error` per the doc. Loader sets `kind`; `parentTableFor(kind)` is hard-coded — never user input. **Shipped (2026-04-23):** [backend/internal/entityrefs/service.go](../../backend/internal/entityrefs/service.go). Surface ended up as `LoadParent` + `InsertEntityStakeholder` + `InsertPageEntityRef` + `CleanupChildren` — `LoadParent` is exposed because callers like bookmarks need the tenant for downstream work (cleaner than re-querying).
- [x] **2.2** Refactor `backend/internal/nav/bookmarks.go` to use the new service for its `page_entity_refs` insert. Behaviour unchanged; just routed through the shared writer. **Shipped:** `Bookmarks.Refs *entityrefs.Service` field, `Pin` now calls `Refs.InsertPageEntityRef`. Sentinel errors re-exported so handler code switching on `nav.ErrEntityNotFound` etc. keeps working unchanged. Full nav test suite green post-refactor.
- [x] **2.3** Implement `cleanupPolymorphicChildren(ctx, tx, kind, id) error` as a registry function. Source of truth is the table in `c_polymorphic_writes.md` — duplicate the map in code with a comment pointing back to the doc. **Shipped as `Service.CleanupChildren`.** Registry is `childRelationshipsFor(kind)` — comment in code points back to the doc. Workspace correctly returns stakeholders only (page_entity_refs CHECK rejects workspace).
- [x] **2.4** Unit tests for `cleanupPolymorphicChildren` covering each parent kind. Use a tx-rollback harness so no live data churn. **Shipped (2026-04-23) in [backend/internal/entityrefs/service_test.go](../../backend/internal/entityrefs/service_test.go).** 10 tests covering parentTableFor, childRelationshipsFor (including workspace regression guard for the Phase 0.5 vocabulary drift), LoadParent (not-found, unknown-kind, cross-tenant, archived, happy-path), and CleanupChildren (unknown-kind, portfolio happy-path with both child tables, workspace skips page_entity_refs). All pass against live DB.
- [x] **2.5** Update `c_polymorphic_writes.md` Go pattern section to reflect the actual service shape (it's currently a sketch). **Done:** "Go pattern" section rewritten to describe the live `entityrefs.Service` API. "Cleanup registry" header points at `CleanupChildren`. "Open gap" section now narrowed to Phase 3 (archive handlers).
...
- [x] **3.1** Audit existing archive handlers for `workspace`, `portfolio`, `product`, `company_roadmap`, `portfolio_item_types`, `execution_item_types`. **Result (2026-04-23):** zero handlers exist in `backend/internal/`. Searches: `grep -rn 'archived_at\s*=' backend/ --include='*.go'` → no hits; `grep -rn 'func.*[Aa]rchive|func.*[Dd]elete.*[Ww]orkspace|...'` → no hits. Every parent kind in the registry is currently un-archivable from Go.
- [x] **3.2** N/A — no existing handlers to wire (see 3.1). Box closed without code change. The work transferred to 3.3 (cap the future).
- [x] **3.3** Cap shipped as a `REGISTRY of archive handlers` comment block above `entityrefs.Service.CleanupChildren` in [`backend/internal/entityrefs/service.go`](../../backend/internal/entityrefs/service.go) — lists the 4 entity kinds with no Go handler yet and states "Each future handler MUST call CleanupChildren in the same tx as its archive UPDATE." Chose the service over per-table write-sites because (a) SQL migration files are immutable history, (b) the registry is the single source of truth that handlers are caching against. The dispatch trigger is named explicitly as the insert-side enforcement; the canary as the post-deploy backstop.
- [x] **3.4** Lifecycle scaffolding shipped at [`backend/internal/entityrefs/lifecycle_test.go`](../../backend/internal/entityrefs/lifecycle_test.go). 8 cells (workspace×stakeholders, portfolio×{stakeholders, page_refs}, product×{stakeholders, page_refs}, company_roadmap×stakeholders, portfolio_item_types×item_type_states, execution_item_types×item_type_states), all `t.Skipf` with the canonical message. File header explains the fill-in pattern for when handlers ship and notes that `portfolio_item`/`execution_item` cells will be added when the parent tables exist. All 8 SKIP, package compiles.
- [x] **3.5** Canary `TestNoPolymorphicOrphans` re-run **2026-04-23**: PASS across `entity_stakeholders`, `item_type_states`, `page_entity_refs` (item_state_history skipped — parent tables not yet in schema). Lifecycle suite: 8 SKIP, 0 FAIL.
