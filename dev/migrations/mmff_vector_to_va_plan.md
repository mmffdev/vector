...
| 44 | `subscription_artifacts` | DROP (blocked) | ⛔ | P5-blocked | 0 rows; LIVE WRITE TARGET — `portfoliomodels/adopt.go:915` INSERTs into it as part of the adoption saga (step 5 of 6). Also DELETEd by `dev_reset.go:115,264`. Blocked on portfoliomodels VA migration (separate PLA). |
...
| 46 | `subscription_portfolio_model_state` | DROP (blocked) | ⛔ | P5-blocked | 0 rows; LIVE WRITE TARGET — `portfoliomodels/adopt.go:586-679` INSERT/UPDATE adoption state (status, adopted_at, adopted_by_user_id, failed step). `service.go:560` SELECTs from it. Core adoption saga state machine. Blocked on portfoliomodels VA migration (separate PLA). |
...
| 48 | `subscription_terminology` | DROP (blocked) | ⛔ | P5-blocked | 0 rows; LIVE WRITE TARGET — `portfoliomodels/adopt.go:940` INSERTs terminology rows as adoption saga step 6. `dev_reset.go:116,263` DELETEs. Blocked on portfoliomodels VA migration (separate PLA). |
| 49 | `subscription_workflow_transitions` | DROP (blocked) | ⛔ | P5-blocked | 0 rows; LIVE WRITE TARGET — `portfoliomodels/adopt.go:884` INSERTs transition rows as adoption saga step 4. `dev_reset.go:117,260` DELETEs. Blocked on portfoliomodels VA migration (separate PLA). |
| 50 | `subscription_workflows` | DROP (blocked) | ⛔ | P5-blocked | 0 rows; LIVE WRITE TARGET — `portfoliomodels/adopt.go:731,845` SELECTs + INSERTs workflow rows as adoption saga step 3. `dev_reset.go:118,261` DELETEs. Blocked on portfoliomodels VA migration (separate PLA). |
