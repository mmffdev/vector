...
- **PLAT1.7 [P2] 🔵 IN FLIGHT** — CP service as OIDC OP with DPoP. Standalone runtime owning identity.
  ...
  > ⚠️ MOSTLY DONE 2026-06-09 — full OIDC impl shipped in slices A–D (commits 7901f8d7 signer/ES384+JWKS, 1b106aec token issuance, 34e77f3b DPoP verify [Vector-byte-identical], 6700e8e2 /authorize→/token→/userinfo). **AC1 ✅** discovery served. **AC2 ✅** auth-code+PKCE mints a cnf.jkt-bound ES384 token (`token_type:"DPoP"`), unbound/mismatched proof rejected (token_flow_test.go, 7 tests green; live cloned-DB integration green). **KMS VERIFIED LIVE** — eu-west-2 ECC_NIST_P384, FIPS 140-3 L3, /jwks kid==x_4oni… (verify-kms.sh). Identity DB cloned (15 tables). REMAINING for full AC closure: **AC3** regenerate SY003 for the platform DB (not done — platform DB is a NEW substrate, SY003 still describes Vector's two DBs); **AC4** grep-prove zero cross-DB FKs (product tables → users by UUID soft-ref) not yet run; **AC5** the two named tests exist by behaviour but under different names (TestTokenFlow_* / TestIssuer_RejectsUnboundToken) — rename or alias to TestCP_DPoP_RejectsUnboundToken + TestCP_PKCE_RequiresVerifier to satisfy verbatim. Interactive login is interim (X-CP-Authenticated-User header, LOUD TODO) → real login is PLAT1.9. token-exchange grant (RFC 8693) is a 501 stub. Self-handover: HANDOVER-SELF-PLAT1.7.md (platform repo).
