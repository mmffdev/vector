...
- **PLAT1.7 [P2] 🔵 IN FLIGHT** — CP service as OIDC OP with DPoP. Standalone runtime owning identity.
  ...
  > ⚠️ MOSTLY DONE 2026-06-09 — full OIDC impl shipped in slices A–D (commits 7901f8d7 signer/ES384+JWKS, 1b106aec token issuance, 34e77f3b DPoP verify [Vector-byte-identical], 6700e8e2 /authorize→/token→/userinfo). **AC1 ✅** discovery served. **AC2 ✅** auth-code+PKCE mints a cnf.jkt-bound ES384 token (`token_type:"DPoP"`), unbound/mismatched proof rejected (token_flow_test.go, 7 tests green; live cloned-DB integration green). **KMS VERIFIED LIVE** — eu-west-2 ECC_NIST_P384, FIPS 140-3 L3, /jwks kid==x_4oni… (verify-kms.sh). Identity DB cloned (15 tables). **AC3 ✅** SY003 regenerated 2026-06-09 with a Platform Identity DB section (15 tables, live row counts, the duplication finding) — live in `dev_reports`. **AC5 ✅** the two verbatim-named RED-GREEN tests `TestCP_DPoP_RejectsUnboundToken` + `TestCP_PKCE_RequiresVerifier` added (commit 5ca5310e, green). **AC4 ❌ OPEN (blocked, see TD-PLATFORM-IDENTITY-CARVEOUT, S2):** the PLAT1.2 clone COPIED identity into the platform DB rather than carving it out — `vector_artefacts` STILL holds its own `users` (60 rows) + **42 FKs**, so "zero cross-DB FKs / UUID soft-ref only" is NOT met. Closing AC4 needs a destructive carve-out migration (drop 42 FKs + the `users` cluster from a live tenant DB) — NOT done autonomously; needs user-approved PLA card. Interactive login is interim (X-CP-Authenticated-User header, LOUD TODO) → real login is PLAT1.9. token-exchange grant (RFC 8693) is a 501 stub. **Slice E (dual-accept) shipped Vector-side** — see PLAT1.9 note. Self-handover: HANDOVER-SELF-PLAT1.7.md (platform repo).
...
- **PLAT1.9 [P2] 🔵 IN FLIGHT** — Vector as first relying party (feature-flagged). Vector authenticates against the CP.
  ...
  - Phase: 3 · Plan: PLA077
  > 🟡 PARTIAL 2026-06-09 — backend dual-accept groundwork landed (PLAT1.7 "slice E", commit 80d63638): `backend/internal/auth/cp_dual_accept.go` adds `ParseAccessTokenDualAccept` — accepts a CP-issued ES384 token (verified via CP JWKS from `CP_JWKS_JSON`) IN ADDITION to legacy HS256, behind flag `CP_AUTH_DUAL_ACCEPT` (default OFF), held to the same iss/aud policy. Behaviour-preserving: legacy HS256 path runs first + wins; the ES384 path can only turn a legacy-reject into an accept, never the reverse. Shadow-run (`CP_AUTH_SHADOW`) records the legacy/cp/both-reject path mix via a swappable `ShadowSink` (in-memory today → cp_shadow_divergence in PLAT1.10/1.12). 8 RED-GREEN tests green, full auth suite green. REMAINING for PLAT1.9 proper: the frontend login→CP redirect, Sentinel hydration from a CP boot endpoint, and the e2e spec — none done yet. This is the token-acceptance half only.
  > 🟢 FE LANDED (flag-gated) 2026-06-09 (vector b57c90c4 + platform 3e4f0c1b) — the frontend RP path is wired behind `NEXT_PUBLIC_CP_AUTH_ENABLED` (default OFF, legacy login untouched): `app/lib/cpAuth.ts` (PKCE+ES256 DPoP, redirects to CP /authorize with `response_mode=form_post`), `/api/cp/callback` route (reads code+state from the POST body — PLA-0053-clean, no URL query) bridging to `/cp/callback` page, `AuthContext.adoptCpSession()` (sets CP token + /auth/me). CP OP gained `response_mode=form_post` so the response never hits the address bar. "Sign in with MMFF Platform" button shows only when the flag is on. 6 cpAuth unit tests + form_post OP test green; full lint suite green. **REMAINING:** (a) real interactive login at the CP (still the X-CP-Authenticated-User dev scaffold); (b) Sentinel boot-endpoint hydration.
  > ✅ DPoP-BINDING + CP-OWNED REFRESH CLOSED 2026-06-09 (vector 657ff47b + platform 2a45657b — PLAT1.12). The two CP-session gaps are gone: **(1)** cpAuth now generates the DPoP keypair non-extractable and persists it to Vector's IDB under the token subject, so the CP token's cnf.jkt matches the key the refresh path signs with (no more revoke-all). **(2)** The CP now OWNS the session lifecycle — `internal/oidc/refresh` issues a rotating, single-use, DPoP-bound, family-revoking refresh token; `/token` gained `grant_type=refresh_token` (DPoP-verified, kill-switch-checked, fail-closed on deactivation). Vector's `refreshCpSession()` rotates against the CP and `AuthContext.refresh()` routes CP sessions there instead of `/auth/refresh`. Tests: refresh store (8) + refresh HTTP flow (6) + Vector cpAuth (9) + e2e (8), all green. Only the durable substrate (in-memory → platform DB refresh_tokens) remains for productionisation.
...
- **PLAT1.12 [P3] 🔵 IN FLIGHT** — Cut over authentication route-group to the CP. First real flip, reversible.
  ...
  - Phase: 5 · Plan: PLA077
  > 🟡 GROUNDWORK LANDED 2026-06-09 (platform 2a45657b + vector 657ff47b) — the CP now OWNS the session lifecycle, the core of the cutover. `internal/oidc/refresh` (rotating/single-use/DPoP-bound/family-revoking refresh tokens) + `/token` grant_type=refresh_token (DPoP-verified, kill-switch-checked); Vector refreshes CP sessions against the CP (refreshCpSession + AuthContext routing), flag-gated NEXT_PUBLIC_CP_AUTH_ENABLED (default OFF, legacy path intact + reversible — AC2). Login events land in the central trail (AC4, via PLAT1.13 audit). **REMAINING for the full flip:** serve the auth route-group from the CP by default once shadow divergence is zero (AC1 — still flag-OFF, not flipped), per-role allow/deny contract tests against the CP-served path (AC3), and retiring the X-CP-Authenticated-User scaffold for real interactive CP login. The mechanism is built; the default-on flip is the deliberate remaining step.
...
- **PLAT1.13 [P3] 🔵 IN FLIGHT** — Central revocation: back-channel logout + RFC 7009. The central kill-switch.
  ...
  - Phase: 5 · Plan: PLA077
  > ✅ AC4 CLOSED 2026-06-09 (platform 04fcaaba) — central audit trail wired: POST /platform/audit GREEN (internal/audit Sink), OP emits a login event per /token mint + revoker emits a revoke event, both into ONE correlation-id'd trail; cross-product SSO Go test asserts "one trail, both products". Only the durable DB-backed sink (in-memory → audit_logs) remains for productionisation.
