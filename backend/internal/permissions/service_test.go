...
//
// Each test provisions two throwaway tenants (so cross-subscription attempts have
...
type subscriptionFix struct {
	subscriptionID    uuid.UUID
	userID      uuid.UUID
	workspaceID uuid.UUID
}

// mkSubscriptionFix provisions a tenant + user + the default workspace.
// The default-workspace seed function does the company_roadmap + workspace
// inserts; we just look up the resulting workspace id.
func mkSubscriptionFix(t *testing.T, pool *pgxpool.Pool, label string) (subscriptionFix, func()) {
	t.Helper()
	...
	if err := pool.QueryRow(ctx, `
		...
		subscriptionID, "perm-test-"+label+"-"+suffix+"@example.com",
	...
	if _, err := pool.Exec(ctx,
		`SELECT provision_subscription_defaults($1, $2)`, subscriptionID, userID,
	); err != nil {
	...
	if err := pool.QueryRow(ctx,
		`SELECT id FROM workspace WHERE subscription_id = $1 AND key_num = 1`, subscriptionID,
	...
	cleanup := func() {
		...
		_, _ = pool.Exec(ctx, `DELETE FROM page_entity_refs WHERE entity_id IN
			(SELECT id FROM portfolio WHERE subscription_id = $1
			 UNION SELECT id FROM product WHERE subscription_id = $1)`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM entity_stakeholders WHERE subscription_id = $1`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM portfolio_item_state WHERE subscription_id = $1`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM execution_item_state WHERE subscription_id = $1`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM item_type_transition_edges WHERE subscription_id = $1`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM item_type_states WHERE subscription_id = $1`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM portfolio_item_types WHERE subscription_id = $1`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM execution_item_types WHERE subscription_id = $1`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM product WHERE subscription_id = $1`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM portfolio WHERE subscription_id = $1`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM workspace WHERE subscription_id = $1`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM company_roadmap WHERE subscription_id = $1`, subscriptionID)
		_, _ = pool.Exec(ctx, `DELETE FROM subscription_sequence WHERE subscription_id = $1`, subscriptionID)
	...
	return subscriptionFix{subscriptionID: subscriptionID, userID: userID, workspaceID: workspaceID}, cleanup
}
...
func TestGrant_RejectsCrossTenantWorkspace(t *testing.T) {
	...
	a, cleanA := mkSubscriptionFix(t, pool, "a")
	defer cleanA()
	b, cleanB := mkSubscriptionFix(t, pool, "b")
	defer cleanB()
...
func TestRevoke_RejectsCrossTenant(t *testing.T) {
	...
	}, b.subscriptionID, b.userID, "")
	if err != nil {
		t.Fatalf("seed grant in B: %v", err)
	}

	// Actor in A tries to revoke B's permission row.
	if err := svc.Revoke(ctx, pB.ID, a.subscriptionID, a.userID, ""); !errors.Is(err, ErrNotFound) {
		t.Fatalf("expected ErrNotFound on cross-subscription revoke, got %v", err)
	}

	// Confirm the row is still there (i.e. the cross-subscription DELETE was a no-op).
	var stillThere bool
	if err := pool.QueryRow(ctx,
	...
	if !stillThere {
		t.Fatalf("row was deleted by cross-subscription Revoke — isolation broken")
	}

	// Sanity: actor in B can revoke their own row.
	if err := svc.Revoke(ctx, pB.ID, b.subscriptionID, b.userID, ""); err != nil {
...
func TestListForUser_ScopedToActorTenant(t *testing.T) {
	...
	if _, err := svc.Grant(ctx, GrantInput{
		UserID: b.userID, WorkspaceID: b.workspaceID, CanView: true,
	}, b.subscriptionID, b.userID, ""); err != nil {
		t.Fatalf("seed grant in B: %v", err)
	}

	// Actor in A asks for B's user permissions — should be empty (existence
	// hidden), not the seeded row.
	got, err := svc.ListForUser(ctx, b.userID, a.subscriptionID)
	if err != nil {
		t.Fatalf("ListForUser: %v", err)
	}
	if len(got) != 0 {
		t.Fatalf("cross-subscription List leaked %d rows: %+v", len(got), got)
	}

	// Sanity: actor in B sees their own user's permissions.
	got, err = svc.ListForUser(ctx, b.userID, b.subscriptionID)
	if err != nil {
...
func TestListForWorkspace_RejectsCrossTenant(t *testing.T) {
	...
	_, err := svc.ListForWorkspace(context.Background(), b.workspaceID, a.subscriptionID)
	if !errors.Is(err, ErrNotFound) {
		t.Fatalf("expected ErrNotFound, got %v", err)
	}
}
