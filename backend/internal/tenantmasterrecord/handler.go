...
// (PLA-0050 / Story 00569 — subscription-tier defaults table).
// Mounts under /_site/tenant-settings; both routes require auth +
// gadmin gate (handled by main.go middlewares). The row is keyed by
// the caller's subscription_id; gadmin-only because tenant defaults
// affect every workspace in the subscription.
...
// GET /_site/tenant-settings — returns the row for the caller's tenant
// (subscription_id from auth context).
...
// PATCH /_site/tenant-settings — partial update. 422 with violations[]
