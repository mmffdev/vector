...
// Run manually:
//
//	BACKEND_ENV=dev go test -v ./internal/workitemsv2/...
//	BACKEND_ENV=dev go test -v -run TestCreateWorkItem ./internal/workitemsv2/...
...
func TestListWorkItems_ReturnsTenantRows(t *testing.T) {
	...
	svc := workitemsv2.NewService(va, nil)

	items, total, err := svc.ListWorkItems(context.Background(), sub, workitemsv2.Filters{Limit: 100})
	if err != nil {
...
func TestListWorkItems_CrossTenantIsolation(t *testing.T) {
	...
	items, total, err := svc.ListWorkItems(context.Background(), unknown, workitemsv2.Filters{Limit: 50})
	if err != nil {
...
func TestListWorkItems_Pagination(t *testing.T) {
	...
	_, total, err := svc.ListWorkItems(ctx, sub, workitemsv2.Filters{Limit: 1000})
	if err != nil {
	...
	p1, _, err := svc.ListWorkItems(ctx, sub, workitemsv2.Filters{Limit: 1, Offset: 0})
	if err != nil {
	...
	if total >= 2 {
		p2, _, err := svc.ListWorkItems(ctx, sub, workitemsv2.Filters{Limit: 1, Offset: 1})
		if err != nil {
...
func TestListWorkItems_ItemTypeFilter(t *testing.T) {
	...
	itemType := "story"
	stories, _, err := svc.ListWorkItems(ctx, sub, workitemsv2.Filters{
		ItemType: &itemType,
		Limit:    100,
	})
	if err != nil {
...
func TestListWorkItems_SortWhitelist(t *testing.T) {
	...
	_, _, err := svc.ListWorkItems(context.Background(), sub, workitemsv2.Filters{
...
func TestNilPool_ReturnsEmpty(t *testing.T) {
	...
	items, total, err := svc.ListWorkItems(ctx, sub, workitemsv2.Filters{Limit: 10})
...
func TestCreateWorkItem_StoresRow(t *testing.T) {
	...
	in := workitemsv2.CreateWorkItemInput{
		ItemType:  "story",
...
func TestCreateWorkItem_TaskRejectsPoints(t *testing.T) {
	...
	pts := 5
	_, err := svc.CreateWorkItem(context.Background(), sub, workitemsv2.CreateWorkItemInput{
		ItemType:    "task",
...
func TestPatchWorkItem_UpdatesTitle(t *testing.T) {
	...
	patched, err := svc.PatchWorkItem(ctx, sub, uuid.MustParse(wi.ID), workitemsv2.PatchWorkItemInput{
		Title: &newTitle,
	})
	if err != nil {
...
func TestPatchWorkItem_DueDate_SetAndClear(t *testing.T) {
	...
	// Set.
	date := "2027-03-15"
	set, err := svc.PatchWorkItem(ctx, sub, id, workitemsv2.PatchWorkItemInput{DueDate: &date})
	if err != nil {
	...
	empty := ""
	cleared, err := svc.PatchWorkItem(ctx, sub, id, workitemsv2.PatchWorkItemInput{DueDate: &empty})
	if err != nil {
...
func TestPatchWorkItem_NotFound(t *testing.T) {
	...
	_, err := svc.PatchWorkItem(context.Background(), uuid.New(), uuid.New(),
		workitemsv2.PatchWorkItemInput{Title: &title})
	if err == nil {
		t.Fatal("expected ErrNotFound, got nil")
	}
}
...
func TestListChildren_ReturnsOnlyDirectChildren(t *testing.T) {
	...
	parent, err := svc.CreateWorkItem(ctx, sub, workitemsv2.CreateWorkItemInput{
		ItemType:  "epic",
		Title:     "parent-epic",
		OwnerID:   ownerID.String(),
		CreatedBy: ownerID.String(),
	...
	child, err := svc.CreateWorkItem(ctx, sub, workitemsv2.CreateWorkItemInput{
		ItemType:  "story",
		Title:     "child-story",
		ParentID:  &parent.ID,
		OwnerID:   ownerID.String(),
