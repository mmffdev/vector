...
const sqlInsertDPoPJTI = `
		INSERT INTO dpop_jti_cache (dpop_jti_cache_jti, dpop_jti_cache_expires_at)
		VALUES ($1, $2)
		ON CONFLICT (dpop_jti_cache_jti) DO NOTHING
		RETURNING xmax
	`
...
const sqlDeleteExpiredDPoPJTIs = `DELETE FROM dpop_jti_cache WHERE dpop_jti_cache_expires_at <= NOW()`
