...
**REJECT — FB1.3.4 (`@dnd-kit` drag + hard-blocked transitions).** Phase 3 at 3/7 (no merge). Brief in § Active rejection — worker fix is a single CSS-block addition to `app/globals.css`, no source code changes needed.
