...
**FB1.2.3 REJECTED — see § Active rejection.** Worker SHA `8b5c342f`. AC 2 fails at runtime: `sqlUpsertCardPrefs` uses `ON CONFLICT ON CONSTRAINT users_flowboard_prefs_user_type_uidx`, but mig 134 created that name as a UNIQUE INDEX, not a CONSTRAINT. PostgreSQL rejects: `constraint "users_flowboard_prefs_user_type_uidx" for table "users_flowboard_prefs" does not exist`. Tests pass because they use a fake service that never touches SQL. One-line fix: switch UPSERT to column-list form `ON CONFLICT (users_flowboard_prefs_user_id, users_flowboard_prefs_artefact_type_id)`.
