...
**Call-site corollary (writing a fetch, not just tracing one).** The same trap bites when you WRITE client code, not only when you trace it: because the artefacts List handler still *accepts* `?meg=`, a grep makes it look like "the way to scope a query to the current node." It is not. The sentinel clamp (`FocusNodeID` + `AllowedSubtreeIDs`) already establishes topology scope server-side, fail-closed via `sentinel.SubtreeClause`. **A clamp-scoped list call passes ONLY its selection filters (e.g. `item_type_id=<typeId>`) — node scope is implicit and non-optional.** `?meg=` is URL transport (`SentinelProvider.setFocus()` writes it; `withForwardedMeg` auto-forwards it on `/work-items` + `/portfolio-items` GETs) — never something to append by hand to "narrow to the current node." If you find yourself adding `&meg=${nodeId}` to a fetch, stop: the clamp already did it. Recurrence: 2026-05-30 — Claude recommended `?meg=` to scope an FLB artefact-preview dropdown right after correctly documenting that `?meg=` has no authority. Knowing the rule and applying it at the keyboard are two different failures; this corollary closes the second.
