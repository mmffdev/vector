...
- **`<report>` skill (`-r` / `-b` / `-s` / `-c` / `-retro`)** → [`.claude/skills/report/SKILL.md`](skills/report/SKILL.md) — umbrella for every narrative report: web research, codebase audit, security audit, single-file dependency trace, retrospective. All flags POST to `/_site/admin/dev/reporting/`; reports land in `mmff_dev.dev_reports` and render on Dev → Reporting. `-retro --auto-loop` is the loop-detector entry point (5-Whys + reversal + ledger + tech-debt promotion); index at [`docs/c_retro_index.md`](../docs/c_retro_index.md). Replaces the retired `<research>`/`<codebase>`/`<sec>`/`<code>`/`<retro>` skills (2026-05-21).
