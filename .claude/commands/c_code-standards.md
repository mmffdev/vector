...
- When building anything for a 'Dev Mode' section of the app, all contents MUST be contained within the root/dev folder
- When adding an element to a page, you MUST check if an avilable CSS class exists already, you MUST NOT create bespoke classes for elements we already have such as Tables, Buttons, Panels, headers etc. Unless strict expression is given by MMFFDev
...
All tables and accorions use the current primative styles from the global.css style sheet, you MUST NEVER create a new class on an item by item basis. The golden rule is reuse existing classes to ensure easy maintenance and standardisation. If you need to use an seperate primative, stop and explain why, asking for guidance.

Unless a Dev Mode modification, in which case, use the root/dev css  intead as the reference pattern, note.. the same rule abpove applies, we use original primatives unless needed, in which case stop and ask for clairty.


We work on a 'do we have the classes needed for styling already' basis, we do not create addition classes unless they are to extend the primative class structure

## Grid / Accordion Standard

When building expandable row grids or accordion layouts, use the CSS class stucture already inside the global.css file, unless a Dev Mode modification, in which case, use the root/dev css  intead as the reference pattern. Parent rows with expand/collapse, child detail panels, stat boxes, search/filter controls, inline actions. Follow the same CSS block naming and `ui-filterbar` / `ui-statbox` integration.

We work on a 'do we have the classes needed for styling already' basis, we do not create addition classes unless they are to extend the primative class structure.

## Grid / Accordion Standard

When building expandable row grids or accordion layouts, 
