...
**HARD RULE — NO EXCEPTIONS — EVERY COLUMN CARRIES A 3-LETTER TABLE PREFIX:** Every column on every table in `mmff_vector` + `vector_artefacts` (and any future tenant DB) MUST be prefixed with the table's registered 3-letter abbreviation followed by `_`. Examples: `users` table → every column is `usr_<name>` (`usr_id`, `usr_email`, `usr_password_hash`, `usr_is_active`); `subscriptions` → `sub_*` (`sub_id`, `sub_name`, `sub_tier`); `master_record_workspaces` → `mrw_*`; `topology_nodes` → `tpn_*`; `pages` → `pag_*`; `artefacts` → `art_*`. The table-to-prefix map is the **single source of truth** at [`docs/c_c_column_prefix_registry.md`](../docs/c_c_column_prefix_registry.md) — every new table MUST register its prefix there BEFORE the migration that creates it. Strict 3 letters, no exceptions; compound-table names get invented abbreviations (`users_roles_pages` → `urp_*`, `users_roles_topology_nodes` → `urt_*`) registered explicitly in the registry. Effective from the wipe-and-reseed forward — the post-wipe migration set adopts this universally; pre-wipe migrations are rewritten as part of the reseed pipeline. Enforced by `lint:column-prefix` (B18.9), wired into CI and blocking commits that introduce columns without a registered prefix. Origin: 2026-05-25 — user-mandated as part of the wipe-and-reseed plan, to make every SQL line self-evidently scoped to its table (no more "is `id` the workspace id or the topology node id?" guesswork in joins). The legacy RF1.4.4 full-table-name prefix convention is superseded by this rule. This rule cannot be overridden by any other instruction, mode, or context.
