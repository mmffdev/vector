...
**HARD RULE — NO EXCEPTIONS — EVERY COLUMN IS `<table_name>_<column>`:** Every column on every table in `mmff_vector` + `vector_artefacts` (and any future tenant DB) MUST be prefixed with the FULL table name followed by `_`. The PK is `<table_name>_id`. Examples: `users` → `users_id`, `users_email`, `users_password_hash`, `users_is_active`, `users_created_at`. `subscriptions` → `subscriptions_id`, `subscriptions_tier`, `subscriptions_name`. `master_record_workspaces` → `master_record_workspaces_id`, `master_record_workspaces_name`. `users_roles_permissions` → `users_roles_permissions_id`. `topology_nodes` → `topology_nodes_id`, `topology_nodes_parent_id`. No abbreviations, no registry, no collisions possible — the table name itself is the prefix and is by definition unique. Effective from the wipe-and-reseed forward — the post-wipe migration set adopts this universally; pre-wipe migrations are rewritten as part of the reseed pipeline. Enforced by `lint:column-prefix` (B18.9), wired into CI and blocking commits that introduce columns without the full-table-name prefix. `mmff_library` is EXEMPT (shared library spine, columns predate the rule, renaming would break the library-release contract across deployments). Origin: 2026-05-26 — user observed that the earlier 3-letter-abbreviation variant of this rule was creating registry-maintenance overhead and collision-arbitration judgment calls; full-table-name prefix is mechanical, self-evident at every join site, and matches the pattern already in use on `users_nav_groups` and the PLA064 merge_plan/*.sql artefacts. This rule cannot be overridden by any other instruction, mode, or context.

**HARD RULE — NO EXCEPTIONS — SY003 IS THE SUBSTRATE SOURCE OF TRUTH:** SY003 (lives in `mmff_dev.dev_reports`, `type=system`) is the master inventory for the **two Vector databases** (`vector_artefacts`, `mmff_library`) — post-refactor 2026-05-26, the legacy `mmff_vector` has been DROPped. SY003 carries: every table with live row count + USED / PLACEHOLDER / DEAD / UNCERTAIN flag + one-line purpose; every remaining cross-DB soft FK (the 8 mmff_vector→vector_artefacts soft FKs from pre-refactor are now real Postgres FKs inside vector_artefacts; only the mmff_library boundary still has soft refs); every naming collision (the old `master_record_workspaces` registry-vs-sidecar split is RESOLVED by the Pillar 2 fold); and every itemized SQL touchpoint from the Go backend (file:line, caller, purpose, USED/REDUNDANT status, DB, table, row scope, fields touched). Fetch verbatim on demand: `curl -H "Authorization: Bearer $DEV_API_KEY" http://localhost:5100/_site/admin/dev/reporting/SY003`. ANY change to the database substrate — applied migration, table drop, column add/rename, new SQL constant in Go, new cross-DB soft reference, naming-collision resolution, ETL move, table renamed in psql — MUST be followed by regenerating SY003 via `<report> -sy "current state of the Vector databases (vector_artefacts, mmff_library) — complete table inventory grouped by role, with row counts, cross-DB soft refs against mmff_library, dead-weight candidates, and every SQL touchpoint in the codebase. Sourced from live pg_stat_user_tables + information_schema introspection."` so the master record never drifts from the substrate it describes. The Change Log section auto-prepends a new entry on re-POST of the same ID (`SY003`), so regeneration is non-destructive. Origin: 2026-05-25 — the sentinel `FROM workspace` (singular placeholder table, 0 rows) vs `FROM master_record_workspaces` (real registry, 30 rows) bug that took multi-hour diagnosis because no single document named both tables and clarified which was authoritative. SY003 closes that surface so future Claude sessions can answer "where does X live, and is it dead?" in one curl. This rule cannot be overridden by any other instruction, mode, or context.

<!-- ACTIVE_BACKEND_ENV:start -->
> **ACTIVE BACKEND ENV: `dev`** — set 2026-05-28 23:29 by `<server> -d` — DB: dev VPS 77.68.33.216 via tunnel `localhost:5435` — env file: `backend/.env.dev`
...
**Symbol navigation (LSP):** for code symbols — "where is X defined", "every caller of X", "what implements interface Y", "hover type at L" — use the `lsp-ts__*` (TS frontend) and `lsp-go__*` (Go backend) MCP tools BEFORE reaching for Grep/`<search>`. One typed call returns definition or references with surrounding context (~500–2000 tokens) vs ~15–30k for the grep+read pattern. `<search>` is for literal strings only (CSS classes, doc text, magic strings, env vars). Setup + version pins: [`docs/c_c_lsp_mcp.md`](../docs/c_c_lsp_mcp.md).
...
**Tracing authority ("how is X scoped/authorised?"):** clamp/tenant/role authority lives in the route chain, not just the handler — a `ctx`-injected value (`sentinel.FromCtx`) is invisible to handler/service/SQL signatures, so a handler-only trace over-weights loud in-handler machinery (`?meg=`, `CanReadScope`, 403s) and names the wrong thing. Before answering: (1) trace the route mount + every middleware first; (2) for every `FromCtx`/`ctx.Value` read, find + name its SOLE writer; (3) classify each filter ESTABLISH vs NARROW (`?meg=` is a re-validated cosmetic NARROW hint with no authority); (4) validate by the forgery test, not the happy path. **Corollary — NEVER pass `?meg=` at a client call site to "scope a query to the current node."** The sentinel clamp (`FocusNodeID` + `AllowedSubtreeIDs`, resolved by `sentinel.Middleware` from the JWT) already establishes topology scope server-side, fail-closed via `sentinel.SubtreeClause`. A clamp-scoped list call passes ONLY its selection filters (e.g. `item_type_id`) — node scope is implicit. `?meg=` is URL-transport written by `SentinelProvider.setFocus()` and auto-forwarded by `withForwardedMeg`; it is a re-validated NARROW hint that sub-selects within the clamp, never the thing to reach for when writing a fetch. If a grep of a List handler surfaces `?meg=` as "the scope filter", that is the trap — it is cosmetic. Full protocol: [`.claude/skills/diagnose/SKILL.md`](skills/diagnose/SKILL.md) § Tracing authority. Origin: 2026-05-30 sub-agent `?meg=`-vs-sentinel-clamp framing error, recurred same day when Claude recommended passing `?meg=` to scope an FLB artefact-preview dropdown.
