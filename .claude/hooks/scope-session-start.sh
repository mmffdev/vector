...
if [[ "${IN_FLIGHT:-0}" -gt 0 ]]; then
  ...
  # WIP advisory — count themes for situational awareness only. The
  # historical 5-item cap was demoted to advisory 2026-05-28 (the
  # "park one before starting new work" nag was firing on every
  # session start and getting in the way more than it was helping).
  # Heuristic: for each `🔵 IN FLIGHT` line above the `# Parked`
  # divider, attribute it to the nearest preceding `## ` header;
  # unique header count = themes in-flight.
  ...
  if [[ "$IN_FLIGHT_THEMES" -gt 0 ]]; then
    MSG="$MSG (${IN_FLIGHT_THEMES} theme(s) in flight)"
  fi
fi
