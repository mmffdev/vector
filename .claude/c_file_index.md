# Codebase File Index (auto-generated)

**Generated:** 2026-08-03 19:16:40
**Generator:** `dev/scripts/gen-file-index.sh`

Map of curated source directories. Use this to **locate the right area before reaching for `<search>`**.
`Grep`/`Glob` direct from here is sub-second; `<search>` should be reserved for unknown territory.

Hand-edited `Purpose:` lines are preserved across regenerations.

## App router & components (TS/TSX)

### app
2 file(s) · key: layout.tsx, page.tsx
Purpose: _(unset)_

### app/(user)
2 file(s) · key: layout.tsx, _shared.tsx
Purpose: _(unset)_

### app/(user)/dev
2 file(s) · key: page.tsx, layout.tsx
Purpose: _(unset)_

### app/(user)/portfolio-items
3 file(s) · key: GridPortfolioItems.tsx, page.tsx, layout.tsx
Purpose: _(unset)_

### app/(user)/portfolio-model
6 file(s) · key: page.tsx, LayersPreviewTable.tsx, types.ts
Purpose: _(unset)_

### app/(user)/risk/__tests__
2 file(s) · key: sentinel.page.risks.test.tsx, page.test.tsx
Purpose: _(unset)_

### app/(user)/scope
4 file(s) · key: scopeColumns.tsx, GridExecution.tsx, scopeTreeData.ts
Purpose: _(unset)_

### app/(user)/user-management
2 file(s) · key: page.tsx, layout.tsx
Purpose: _(unset)_

### app/(user)/value-sprint-review
3 file(s) · key: GridSprintReview.tsx, page.tsx, sprintReviewTreeData.ts
Purpose: _(unset)_

### app/(user)/vector-admin/api-manager
2 file(s) · key: page.tsx, layout.tsx
Purpose: _(unset)_

### app/(user)/vector-admin/api-manager/webhooks
2 file(s) · key: page.tsx, WebhookForm.tsx
Purpose: _(unset)_

### app/(user)/work-items
3 file(s) · key: GridWorkItems.tsx, page.tsx, layout.tsx
Purpose: _(unset)_

### app/(user)/work-items-2
2 file(s) · key: page.tsx, layout.tsx
Purpose: _(unset)_

### app/(user)/workspace-admin/artefacts
2 file(s) · key: layout.tsx, page.tsx
Purpose: _(unset)_

### app/components
80 file(s) · key: SprintBurndownChart.tsx, TaskBurndownChart.tsx, useFlowStatesByType.ts
Purpose: _(unset)_

### app/components/ArtefactCreateFlyout
2 file(s) · key: index.tsx, buildCreateRequests.ts
Purpose: _(unset)_

### app/components/ArtefactInlineForm
9 file(s) · key: useParentCandidates.ts, types.ts, ArtefactInlineForm.tsx
Purpose: _(unset)_

### app/components/CustomFields
4 file(s) · key: CustomFieldEditForm.tsx, TypeBindingsPicker.tsx, CustomFieldsPageBody.tsx
Purpose: _(unset)_

### app/components/DependencyMap
2 file(s) · key: DependencyMapOverlay.tsx, usePersistedDependencyMap.ts
Purpose: _(unset)_

### app/components/DependencyMap/MapViews
5 file(s) · key: types.ts, MapViewSugiyama.tsx, MapViewDagre.tsx
Purpose: _(unset)_

### app/components/FlowBoard
3 file(s) · key: p_FlowBoard.tsx, loader.ts, registry.ts
Purpose: _(unset)_

### app/components/FlowBoard/__tests__
8 file(s) · key: p_FlowBoard.test.tsx, useFlowBoardData.test.ts, transitions.test.ts
Purpose: _(unset)_

### app/components/FlowBoard/card
2 file(s) · key: CardFieldRenderer.tsx, BoardCard.tsx
Purpose: _(unset)_

### app/components/FlowBoard/columns
2 file(s) · key: BoardColumnHeader.tsx, BoardColumn.tsx
Purpose: _(unset)_

### app/components/FlowBoard/hooks
5 file(s) · key: useFlowBoardData.ts, usePatchArtefactFlowState.ts, useNodeMembership.ts
Purpose: _(unset)_

### app/components/FlowBoard/settings
2 file(s) · key: WipSettingsModal.tsx, WipGearButton.tsx
Purpose: _(unset)_

### app/components/FormLayoutBuilder
10 file(s) · key: FormBuilderShell.tsx, mergeTransitions.ts, FormLayoutRuntime.tsx
Purpose: _(unset)_

### app/components/FormLayoutBuilder/__tests__
4 file(s) · key: undoRedo.test.ts, mergeTransitions.test.ts, bandLayout.test.tsx
Purpose: _(unset)_

### app/components/Grid
17 file(s) · key: Grid__Tree.tsx, Grid__Tree_Row.tsx, Grid__SprintBoundary.tsx
Purpose: _(unset)_

### app/components/Grid/__tests__
6 file(s) · key: Grid__SprintBoundary.test.tsx, useSweepSelect.test.ts, Grid__Tree_Lines.test.tsx
Purpose: _(unset)_

### app/components/MapRelationship3D
2 file(s) · key: index.tsx, types.ts
Purpose: _(unset)_

### app/components/ObjectTreeV2
3 file(s) · key: p_ObjectTree.tsx, registry.ts, loader.ts
Purpose: _(unset)_

### app/components/ObjectTreeV2/adapters
3 file(s) · key: customFieldsAdapter.tsx, workItemsAdapter.tsx, types.ts
Purpose: _(unset)_

### app/components/ObjectTreeV2/hooks
4 file(s) · key: useObjectTreeWindow.ts, useColumnCatalogue.ts, useObjectTreeFacets.ts
Purpose: _(unset)_

### app/components/ObjectTreeV2/kinds
2 file(s) · key: ActionBar.tsx, DenseGridHeader.tsx
Purpose: _(unset)_

### app/components/ObjectTreeV2/sheets
2 file(s) · key: CreateCustomFields.tsx, ObjectTreeBulkCreateSheet.tsx
Purpose: _(unset)_

### app/components/RichTextField
3 file(s) · key: index.tsx, RichTextField.tsx, RichTextToolbar.tsx
Purpose: _(unset)_

### app/components/SavedViews
8 file(s) · key: PageSavedViewsControl.tsx, types.ts, useSavedViews.ts
Purpose: _(unset)_

### app/components/WorkItemRelations
3 file(s) · key: RelationsGraph.tsx, RelationsSidebar.tsx, RelationsToolbar.tsx
Purpose: _(unset)_

### app/components/__tests__
8 file(s) · key: NavigationPie.test.tsx, AddressDevtool.sentinel.test.tsx, p_ObjectTree.test.tsx
Purpose: _(unset)_

### app/components/charts/sprint
4 file(s) · key: ForecastMarker.tsx, buildBurndownView.ts, buildTaskBurndownView.ts
Purpose: _(unset)_

### app/components/charts/sprint/__tests__
3 file(s) · key: buildBurndownView.test.ts, buildTaskBurndownView.test.ts, axisScale.test.ts
Purpose: _(unset)_

### app/components/flow-rules
5 file(s) · key: StateRail.tsx, OrbitCanvas.tsx, OrbitView.tsx
Purpose: _(unset)_

### app/components/topology
18 file(s) · key: ContextMenu.tsx, ConfirmModal.tsx, useScopedTopologyNodes.ts
Purpose: _(unset)_

### app/contexts
12 file(s) · key: AuthContext.tsx, DomRegistryContext.tsx, NavPrefsContext.tsx
Purpose: _(unset)_

### app/contexts/__tests__
5 file(s) · key: AuthContext.no-cascade.test.tsx, AuthContext.channel-populates-user.test.tsx, AuthContext.
Purpose: _(unset)_

### app/featuretests/__tests__
4 file(s) · key: f2_active_workspace.test.tsx, f9_priority_chip_customisation.test.tsx, f6_status_rename_in
Purpose: _(unset)_

### app/hooks
18 file(s) · key: useNextSprint.ts, useTaskMetrics.ts, usePageTitle.ts
Purpose: _(unset)_

### app/hooks/__tests__
2 file(s) · key: useStepUpAction.test.tsx, useKeyboardGrid.test.tsx
Purpose: _(unset)_

### app/lib
27 file(s) · key: cpAuth.ts, workTypeTier.ts, artefactTypesApi.ts
Purpose: _(unset)_

### app/lib/__tests__
10 file(s) · key: cpAuth.test.ts, workTypeTier.test.ts, dpop.test.ts
Purpose: _(unset)_

### app/lib/apiSite
4 file(s) · key: sprintMetrics.ts, taskMetrics.ts, index.ts
Purpose: _(unset)_

### app/login
2 file(s) · key: page.tsx, layout.tsx
Purpose: _(unset)_

### app/redesign
3 file(s) · key: ShellContext.tsx, page.tsx, layout.tsx
Purpose: _(unset)_

### app/redesign/components
9 file(s) · key: RedesignTopBar.tsx, nav_primary_rail_2.tsx, RedesignShell.tsx
Purpose: _(unset)_

### app/sentinel
5 file(s) · key: SentinelProvider.tsx, sentinel_api.ts, types.ts
Purpose: _(unset)_

### app/user/notifications
2 file(s) · key: layout.tsx, page.tsx
Purpose: _(unset)_

### _Single-file dirs (rolled up)_
- app/(overlay)
- app/(overlay)/topology
- app/(user)/admin
- app/(user)/admin/roles
- app/(user)/artefacts
- app/(user)/backlog
- app/(user)/dashboard
- app/(user)/dependencies
- app/(user)/dev/[tab]
- app/(user)/dev/library
- app/(user)/favourites
- app/(user)/library-releases
- app/(user)/milestones
- app/(user)/my-vista
- app/(user)/p/[id]/[[...vid]]
- app/(user)/planning
- app/(user)/portfolio
- app/(user)/portfolio-items/__tests__
- app/(user)/portfolio-model/__tests__
- app/(user)/portfolio-model/custom
- app/(user)/portfolio-settings
- app/(user)/releases
- app/(user)/risk
- app/(user)/scope/__tests__
- app/(user)/sprints
- app/(user)/table-harness
- app/(user)/user-management/permissions
- app/(user)/user-management/users
- app/(user)/value-flow
- app/(user)/value-sprint
- app/(user)/value-sprint-board
- app/(user)/value-status
- app/(user)/vector-admin
- app/(user)/vector-admin/__tests__
- app/(user)/vector-admin/api-manager/asset-register
- app/(user)/vector-admin/tenant-settings
- app/(user)/work-items/__tests__
- app/(user)/workspace-admin
- app/(user)/workspace-admin/__tests__
- app/(user)/workspace-admin/artefact-types
- app/(user)/workspace-admin/artefacts/artefact-types
- app/(user)/workspace-admin/artefacts/flow-states-v2
- app/(user)/workspace-admin/artefacts/transition-rules
- app/(user)/workspace-admin/cost-centres
- app/(user)/workspace-admin/custom-fields
- app/(user)/workspace-admin/custom-fields/[id]
- app/(user)/workspace-admin/custom-fields/form-designer
- app/(user)/workspace-admin/flow-states
- app/(user)/workspace-admin/flow-states-v2
- app/(user)/workspace-admin/metrics
- app/(user)/workspace-admin/portfolio-model
- app/(user)/workspace-admin/topology
- app/(user)/workspace-admin/topology-map
- app/(user)/workspace-admin/topology-map/__tests__
- app/(user)/workspace-admin/topology/__tests__
- app/(user)/workspace-admin/transition-rules
- app/(user)/workspace-admin/work-items
- app/(user)/workspace-admin/workspace-details
- app/(user)/workspace-admin/workspaces
- app/api/cp/callback
- app/api/dev/api-changelog
- app/api/dev/code
- app/api/dev/go-test
- app/api/dev/library
- app/api/dev/library/file
- app/api/dev/memory-reports
- app/api/dev/operations
- app/api/dev/plans
- app/api/dev/research
- app/api/dev/retros
- app/api/dev/scope
- app/api/dev/scope/stream
- app/api/dev/security-audits
- app/api/dev/services
- app/change-password
- app/components/ArtefactCreateFlyout/__tests__
- app/components/ArtefactInlineForm/__tests__
- app/components/ArtefactTypeCreateFlyout
- app/components/ArtefactTypeCreateFlyout/__tests__
- app/components/Canvas
- app/components/DataContainer
- app/components/FullscreenCanvasOverlay
- app/components/ObjectTree
- app/components/ObjectTreeV2/__tests__
- app/components/ObjectTreeV2/configs
- app/components/ObjectTreeV2/flyouts
- app/components/ObjectTreeV2/plugins
- app/components/RadialPillMenu
- app/components/TimeboxInlineForm
- app/components/TimeboxObjectTree
- app/components/catalogue/c_circular_additor
- app/cp/callback
- app/help/[id]
- app/lib/shared/topology
- app/lib/shared/topology/__tests__
- app/login/reset
- app/login/reset/confirm
- app/sentinel/__tests__
- app/store/shared
- app/user
- app/user/account-settings
- app/user/account-settings/mfa
- app/user/account-settings/sessions
- app/user/navigation
- app/user/notifications/notifications
- app/user/notifications/settings

## Backend Go services

### backend/cmd/auditidx
1 file(s) · key: main.go
Purpose: _(unset)_

### backend/cmd/backfill-artefact-types
1 file(s) · key: main.go
Purpose: _(unset)_

### backend/cmd/cli
1 file(s) · key: main.go
Purpose: _(unset)_

### backend/cmd/cli/client
1 file(s) · key: client.go
Purpose: _(unset)_

### backend/cmd/cli/commands/apikeys
1 file(s) · key: apikeys.go
Purpose: _(unset)_

### backend/cmd/cli/commands/auth
3 file(s) · key: me.go, logout.go, login.go
Purpose: _(unset)_

### backend/cmd/cli/commands/flows
1 file(s) · key: flows.go
Purpose: _(unset)_

### backend/cmd/cli/commands/portfolios
1 file(s) · key: portfolios.go
Purpose: _(unset)_

### backend/cmd/cli/commands/workitems
1 file(s) · key: workitems.go
Purpose: _(unset)_

### backend/cmd/cli/commands/workspaces
1 file(s) · key: workspaces.go
Purpose: _(unset)_

### backend/cmd/cli/printer
1 file(s) · key: printer.go
Purpose: _(unset)_

### backend/cmd/cli/session
1 file(s) · key: session.go
Purpose: _(unset)_

### backend/cmd/encsecret
1 file(s) · key: main.go
Purpose: _(unset)_

### backend/cmd/migrate
1 file(s) · key: main.go
Purpose: _(unset)_

### backend/cmd/server
2 file(s) · key: main.go, rules_producer_hook.go
Purpose: _(unset)_

### backend/dev/scripts
1 file(s) · key: seed_workspace.go
Purpose: _(unset)_

### backend/internal/addressables
9 file(s) · key: service.go, handler.go, service_test.go
Purpose: _(unset)_

### backend/internal/alerting
2 file(s) · key: alerting_test.go, alerting.go
Purpose: _(unset)_

### backend/internal/apikeys
11 file(s) · key: middleware_scopes_test.go, handler_revoke_test.go, dev.go
Purpose: _(unset)_

### backend/internal/artefactitems
26 file(s) · key: types.go, sql.go, service_rank_placement_test.go
Purpose: _(unset)_

### backend/internal/artefactpriorities
4 file(s) · key: service.go, sql.go, handler.go
Purpose: _(unset)_

### backend/internal/artefacttypes
9 file(s) · key: insert_layer_test.go, insert_layer.go, seed_test.go
Purpose: _(unset)_

### backend/internal/audit
1 file(s) · key: audit.go
Purpose: _(unset)_

### backend/internal/auth
28 file(s) · key: middleware.go, cp_dual_accept_test.go, cp_dual_accept.go
Purpose: _(unset)_

### backend/internal/bootstatus
1 file(s) · key: bootstatus.go
Purpose: _(unset)_

### backend/internal/cache
1 file(s) · key: cache.go
Purpose: _(unset)_

### backend/internal/costcentres
4 file(s) · key: service.go, sql.go, handler.go
Purpose: _(unset)_

### backend/internal/cspreport
3 file(s) · key: sql.go, handler.go, service.go
Purpose: _(unset)_

### backend/internal/custompages
2 file(s) · key: service.go, handler.go
Purpose: _(unset)_

### backend/internal/db
1 file(s) · key: db.go
Purpose: _(unset)_

### backend/internal/dbinvariants
1 file(s) · key: orphans_test.go
Purpose: _(unset)_

### backend/internal/dependencies
7 file(s) · key: sql.go, service.go, handler_test.go
Purpose: _(unset)_

### backend/internal/devreports
5 file(s) · key: types.go, types_test.go, sql.go
Purpose: _(unset)_

### backend/internal/devtools
8 file(s) · key: spine.go, sql.go, http.go
Purpose: _(unset)_

### backend/internal/erd
8 file(s) · key: types.go, sql_test.go, sql.go
Purpose: _(unset)_

### backend/internal/errorsreport
5 file(s) · key: handler_test.go, sql.go, service.go
Purpose: _(unset)_

### backend/internal/featuretests
8 file(s) · key: f3_dto_slot_test.go, helpers_test.go, f7_priority_substrate_test.go
Purpose: _(unset)_

### backend/internal/fields
9 file(s) · key: writer_test.go, handler.go, bindings_integration_test.go
Purpose: _(unset)_

### backend/internal/flowboard
5 file(s) · key: sql.go, service.go, handler_test.go
Purpose: _(unset)_

### backend/internal/flows
13 file(s) · key: seed_test.go, seed.go, spine.go
Purpose: _(unset)_

### backend/internal/formlayouts
6 file(s) · key: types.go, handler.go, service.go
Purpose: _(unset)_

### backend/internal/geo
1 file(s) · key: geo.go
Purpose: _(unset)_

### backend/internal/httperr
1 file(s) · key: httperr.go
Purpose: _(unset)_

### backend/internal/librarydb
9 file(s) · key: releases_test.go, grants_test.go, fetch_test.go
Purpose: _(unset)_

### backend/internal/libraryreleases
6 file(s) · key: handler_test.go, sql.go, service.go
Purpose: _(unset)_

### backend/internal/lintchecks
5 file(s) · key: sentinel_clamp_test.go, sentinel_clamp_fixture_test.go, sql_placement_test.go
Purpose: _(unset)_

### backend/internal/logger
3 file(s) · key: middleware.go, authfailure.go, logger.go
Purpose: _(unset)_

### backend/internal/lookups
4 file(s) · key: sql.go, handler.go, service.go
Purpose: _(unset)_

### backend/internal/mentions
5 file(s) · key: service.go, sql.go, dto.go
Purpose: _(unset)_

### backend/internal/messaging/email
7 file(s) · key: types.go, transport.go, templates.go
Purpose: _(unset)_

### backend/internal/nav
9 file(s) · key: catalog.go, sql.go, service_test.go
Purpose: _(unset)_

### backend/internal/navmap
5 file(s) · key: service_test.go, service.go, handler.go
Purpose: _(unset)_

### backend/internal/notifications
12 file(s) · key: sql.go, stream.go, stream_problemjson_test.go
Purpose: _(unset)_

### backend/internal/notifications/broker
3 file(s) · key: rabbit.go, noop.go, broker.go
Purpose: _(unset)_

### backend/internal/notifications/dispatchers
3 file(s) · key: inapp.go, sse.go, email.go
Purpose: _(unset)_

### backend/internal/notifications/resolvers
2 file(s) · key: sql.go, artefactitems.go
Purpose: _(unset)_

### backend/internal/notifications/rules
7 file(s) · key: sql.go, types.go, service.go
Purpose: _(unset)_

### backend/internal/notifications/v2/broadcast
6 file(s) · key: service_test.go, service.go, resolver_test.go
Purpose: _(unset)_

### backend/internal/notifications/v2/broker
6 file(s) · key: broker.go, topology.go, rabbit.go
Purpose: _(unset)_

### backend/internal/notifications/v2/domain
3 file(s) · key: event_test.go, event.go, delivery.go
Purpose: _(unset)_

### backend/internal/notifications/v2/pipeline
14 file(s) · key: prefs.go, pipeline.go, types.go
Purpose: _(unset)_

### backend/internal/notifications/v2/producer
3 file(s) · key: producer.go, dbproducer_test.go, dbproducer.go
Purpose: _(unset)_

### backend/internal/notifications/v2/relay
5 file(s) · key: relay.go, sweeper_test.go, sweeper.go
Purpose: _(unset)_

### backend/internal/notifications/v2/rules
9 file(s) · key: types.go, service_test.go, service.go
Purpose: _(unset)_

### backend/internal/notifications/v2/templates
4 file(s) · key: service_test.go, service.go, interpolate_test.go
Purpose: _(unset)_

### backend/internal/pageaccess
3 file(s) · key: sql.go, resolver.go, handler.go
Purpose: _(unset)_

### backend/internal/permissions
5 file(s) · key: sql.go, catalogue.go, resolver.go
Purpose: _(unset)_

### backend/internal/polymorphicrefs
4 file(s) · key: service.go, sql.go, service_test.go
Purpose: _(unset)_

### backend/internal/portfolio
4 file(s) · key: sql.go, master_record_handler_test.go, master_record_service.go
Purpose: _(unset)_

### backend/internal/portfoliomodels
35 file(s) · key: sql.go, scope_resolver_matrix_test.go, adopt_work_types_test.go
Purpose: _(unset)_

### backend/internal/ranking
7 file(s) · key: service_integration_test.go, position.go, service.go
Purpose: _(unset)_

### backend/internal/realtime
14 file(s) · key: session_sweeper_test.go, sql.go, client.go
Purpose: _(unset)_

### backend/internal/roles
7 file(s) · key: service.go, cache_invalidation_test.go, handler_test.go
Purpose: _(unset)_

### backend/internal/roletypes
1 file(s) · key: models.go
Purpose: _(unset)_

### backend/internal/savedviews
8 file(s) · key: service_test.go, service.go, types.go
Purpose: _(unset)_

### backend/internal/search
3 file(s) · key: handler_test.go, handler.go, service.go
Purpose: _(unset)_

### backend/internal/searchworker
2 file(s) · key: sql.go, worker.go
Purpose: _(unset)_

### backend/internal/secrets
4 file(s) · key: secrets_test.go, secrets.go, get_test.go
Purpose: _(unset)_

### backend/internal/security
10 file(s) · key: csrf_test.go, csrf.go, mfaremember_test.go
Purpose: _(unset)_

### backend/internal/sentinel
10 file(s) · key: types.go, ctx.go, middleware.go
Purpose: _(unset)_

### backend/internal/shared/topology
2 file(s) · key: walker_test.go, walker.go
Purpose: _(unset)_

### backend/internal/sprintmetrics
11 file(s) · key: projection.go, types.go, projection_test.go
Purpose: _(unset)_

### backend/internal/taskmetrics
9 file(s) · key: projection.go, types.go, forecast_test.go
Purpose: _(unset)_

### backend/internal/tenantmasterrecord
4 file(s) · key: service_test.go, sql.go, service.go
Purpose: _(unset)_

### backend/internal/timeboxmilestones
4 file(s) · key: service.go, handler.go, sql.go
Purpose: _(unset)_

### backend/internal/timeboxreleases
5 file(s) · key: service.go, handler.go, sql.go
Purpose: _(unset)_

### backend/internal/timeboxsprints
8 file(s) · key: handler_test.go, service.go, handler.go
Purpose: _(unset)_

### backend/internal/topology
9 file(s) · key: service.go, sql.go, commands.go
Purpose: _(unset)_

### backend/internal/topologyclamp
2 file(s) · key: clamp_sql_test.go, clamp_sql.go
Purpose: _(unset)_

### backend/internal/transport
1 file(s) · key: transport.go
Purpose: _(unset)_

### backend/internal/usermessages
1 file(s) · key: messages.go
Purpose: _(unset)_

### backend/internal/users
8 file(s) · key: sql.go, prefs_problemjson_test.go, prefs.go
Purpose: _(unset)_

### backend/internal/usertaborder
2 file(s) · key: service.go, handler.go
Purpose: _(unset)_

### backend/internal/vectorfields
4 file(s) · key: service.go, sql.go, types.go
Purpose: _(unset)_

### backend/internal/webhooks
5 file(s) · key: worker.go, sql.go, service.go
Purpose: _(unset)_

### backend/internal/workspacemasterrecord
6 file(s) · key: sql.go, service_inheritance_test.go, inheritance_wiring.go
Purpose: _(unset)_

### backend/internal/workspaceresolver
2 file(s) · key: resolver.go, sql.go
Purpose: _(unset)_

### backend/internal/workspaces
10 file(s) · key: service_test.go, crossdb.go, handler_test.go
Purpose: _(unset)_

## Dev tooling

### dev/pages
30 file(s) · key: DevShortcutsPanel.tsx, DevScopeActionsPanel.tsx, DevPage.tsx
Purpose: _(unset)_

### dev/pages/V2A
3 file(s) · key: filterState.ts, filterRegistry.ts, FilterRail.tsx
Purpose: _(unset)_

### dev/scripts
64 file(s) · key: backup-on-push.sh, lint_cross_db_writer_test.py, lint_artefact_read_sentinel.py
Purpose: _(unset)_

### dev/scripts/backup/lib
4 file(s) · key: common.sh, rotate.sh, preflight.sh
Purpose: _(unset)_

### dev/scripts/backup/producers
6 file(s) · key: 50_plesk.sh, 30_rabbitmq.sh, 60_opt_configs.sh
Purpose: _(unset)_

### dev/tests/playwright
6 file(s) · key: sentinel-meg-persistence.spec.ts, playwright.sentinel.config.ts, work-item-relations.spec.
Purpose: _(unset)_

### _Single-file dirs (rolled up)_
- dev/components
- dev/pages/__tests__
- dev/scripts/backup
- dev/scripts/backup/launchd
- dev/store/ui_apps/ui_app_iconbrowser

## Dev fixtures & data

### dev/audits
3 file(s) · key: codegraph.json, role-page-grants.json, api-touchpoints.json
Purpose: _(unset)_

### dev/fixtures/shared/topology
6 file(s) · key: single-root-deep.json, orphan-drop.json, multi-root-forest.json
Purpose: _(unset)_

### dev/plans
47 file(s) · key: PLA-0055.json, PLA-0054.json, PLA-0053.json
Purpose: _(unset)_

### dev/registries
20 file(s) · key: sql_in_sqlfile_exempt.json, cross_db_writer_test_exempt.json, artefact_read_sentinel_exemp
Purpose: _(unset)_

### dev/reports
1343 file(s) · key: 20260610-225126-addressables.json, 20260610-190105-addressables.json, 20260610-185946-addr
Purpose: _(unset)_

### dev/research
59 file(s) · key: R058.json, R057.json, R056.json
Purpose: _(unset)_

### dev/scripts
13 file(s) · key: backfill_spike_flow.sql, backfill_sprint_burn_added.sql, backfill_task_burn_added.sql
Purpose: _(unset)_

### _Single-file dirs (rolled up)_
- dev/code
- dev/operations
- dev/retros
- dev/scripts/backup
- dev/security-audits
- dev/tests/playwright/test-results

## Database schema

### db/mmff_dev/schema
4 file(s) · key: 004_dev_reports_add_architecture.sql, 002_dev_reports.sql, 003_dev_reports_add_system.sql
Purpose: _(unset)_

### db/mmff_dev/schema/down
3 file(s) · key: 004_dev_reports_add_architecture_DOWN.sql, 003_dev_reports_add_system_DOWN.sql, 002_dev_re
Purpose: _(unset)_

### db/mmff_library/schema
13 file(s) · key: 011_layer_tag_definitions.sql, 009_fix_layer_sort_order.sql, 014_rename_library_RF1_4_2.sq
Purpose: _(unset)_

### db/mmff_library/schema/seed
5 file(s) · key: 005_layer_tag_definitions.sql, 004_portfolio_templates.sql, 003_extra_models.sql
Purpose: _(unset)_

### db/mmff_vector/schema
265 file(s) · key: 266_dev_erd_page.sql, 264_users_column_prefix_RF1_5_7.sql, 265_fix_updated_at_triggers_use
Purpose: _(unset)_

### db/mmff_vector/schema/down
74 file(s) · key: 266_dev_erd_page_DOWN.sql, 264_users_column_prefix_RF1_5_7_DOWN.sql, 265_fix_updated_at_tr
Purpose: _(unset)_

### db/ops
1 file(s) · key: cleanup_perm_test_tenants.sql
Purpose: _(unset)_

### db/seed
5 file(s) · key: 002_work_items_poc.sql, 010_master_reset.sql, 003_load_test_work_items_DOWN.sql
Purpose: _(unset)_

### db/vector_artefacts/dev-seeds
1 file(s) · key: seed_risks.sql
Purpose: _(unset)_

### db/vector_artefacts/schema
175 file(s) · key: 181_artefact_types_parent_columns.sql, 180_work_items_2_nav_page.sql, 178_metrics_page.sql
Purpose: _(unset)_

### db/vector_artefacts/schema/down
107 file(s) · key: 181_artefact_types_parent_columns_DOWN.sql, 180_work_items_2_nav_page_DOWN.sql, 178_metric
Purpose: _(unset)_

### db/vector_artefacts/schema/merge_plan
3 file(s) · key: orphan_remediation.sql, subscriptions_merged.sql, master_record_workspaces_merged.sql
Purpose: _(unset)_

### db/vector_artefacts/schema/seed
1 file(s) · key: 01_work_items_fixture.sql
Purpose: _(unset)_

## Documentation indexes

### .
14 file(s) · key: BACKLOG.md, README.md, 2026-06-10.md
Purpose: _(unset)_

### docs
81 file(s) · key: c_tech_debt.md, c_c_lint_rules.md, c_c_tech_debt_completed.md
Purpose: _(unset)_

### docs/Database
1 file(s) · key: Audit_202605280054.md
Purpose: _(unset)_

### docs/Security/Sentinel
5 file(s) · key: sentinel_revision_history.md, sentinel_tests_log.md, sentinel_backlog.md
Purpose: _(unset)_

### docs/Varlock
1 file(s) · key: findings.md
Purpose: _(unset)_

### docs/notes
7 file(s) · key: agent_workspace.md, rick_notes_api.md, mmffdev_builder_brief.md
Purpose: _(unset)_

### docs/superpowers/plans
33 file(s) · key: 2026-06-07-sprint-sweep-rebuild.md, 2026-06-07-sprint-boundary-chrome.md, 2026-06-07-custo
Purpose: _(unset)_

### docs/superpowers/specs
38 file(s) · key: 2026-06-07-sprint-velocity-line-design.md, 2026-06-07-portfolio-items-grid-swap-design.md,
Purpose: _(unset)_

