...
DELETE FROM portfolio_item_state    WHERE tenant_id IN (SELECT id FROM _perm_test_tenants);
DELETE FROM execution_item_state    WHERE tenant_id IN (SELECT id FROM _perm_test_tenants);
