...
// Tenant Settings page (PLA-0050) — single-record editor for
// master_record_tenants (subscription-tier defaults). Workspaces in this
// subscription inherit these values when their own master_record_workspaces
// row carries NULL — inheritance read-path is cleanup-register Story 3,
// not yet implemented; for now these defaults are display-only as far as
// workspace pages are concerned.
//
// Backend: backend/internal/tenantmasterrecord (PATCH validates server-side
// and returns 422 with violations[] on failure). Wire shape in
...
export default function TenantSettingsPage() {
  ...
  const onAccept = useCallback(async () => {
    ...
    try {
      ...
      // Tenant settings DO NOT mutate the TenantContext (which holds
      // workspace-tier data — see header comment).
