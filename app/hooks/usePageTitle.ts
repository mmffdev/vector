...
export function usePageTitle(): { sectionLabel: string; pageLabel: string; leafLabel: string; full: string } {
  ...
  // NOTE: the "<Node> + " sentinel-focus-node prefix is applied by the
  // single header renderer (RedesignTopBar), not here. Prefixing in both
  // places stacked the node name twice ("Retail Banking + Retail Banking
  // + Personal · …"). This hook returns the unprefixed title.
