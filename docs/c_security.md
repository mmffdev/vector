...
- **WebSocket connections:** the registry sweeper runs every `WS_SESSION_CHECK_INTERVAL` (default 30s) plus an immediate-close enqueue from the revoke endpoint. Worst-case lag = sweeper interval if the immediate-close signal is dropped (e.g. process restart between revoke commit and sweep). Implementation: [`backend/internal/realtime/session_registry.go`](../backend/internal/realtime/session_registry.go) (in-memory `sid → close hook` table), [`backend/internal/realtime/session_sweeper.go`](../backend/internal/realtime/session_sweeper.go) (ticker + batched `WHERE users_sessions_id = ANY($1)` query, close codes 4001 revoked / 4002 idle-expired), [`backend/internal/realtime/client.go`](../backend/internal/realtime/client.go) `ServeWS` (register on accept, deregister on close), and `Hub.CloseSession(sid, code, reason)` exposed as the public immediate-close surface for the future revoke endpoint. Frontend mirror: [`app/lib/wsClose.ts`](../app/lib/wsClose.ts) routes 4001 → `hardLogout("session_revoked")`, 4002 → `hardLogout("session_idle_expired")` so the user sees the same banner the HTTP path produces. Contract pinned by `TestWSSessionSweeper_RevokeClosesConnection` and `TestWSSessionSweeper_ImmediateCloseFiresWithoutWaitingForSweep` in `backend/internal/realtime/session_sweeper_test.go` (B16.8.12).
