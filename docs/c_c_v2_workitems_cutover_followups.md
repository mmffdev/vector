...
| ranking NOTIFY trigger not re-attached to `vector_artefacts.artefacts` | live drag-and-drop currently fires on `obj_work_items`; v2 list has no NOTIFY emitter yet | when WORK_ITEMS_V2=true on dev and DnD breaks | _pending fill_ | **RESOLVED 2026-05-08** — rank service retargeted to `artefacts` via `vaPool`; `obj_work_items` NOTIFY trigger moot (table dropped); realtime NOTIFY on `artefacts` deferred to PLA-0023 follow-up if needed |
...
| `obj_work_items` + per-type sibling tables not dropped | keep them live for 7+ days post-cutover for safe rollback; drop is a separate plan | 7 days post WORK_ITEMS_V2=true on dev with zero rollback — trigger active 2026-05-07 | _pending fill_ | **RESOLVED 2026-05-08** — migration 137 dropped both tables in dev; backup taken before drop |
