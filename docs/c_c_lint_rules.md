...
| `lint:column-prefix-convention` | `dev/scripts/lint_column_prefix_convention.py` | `column_prefix_exempt.json` | warn-only scanner for §2.3 violations — every column on a renamed (§2.6 root-family) table must carry the table-name prefix; pre-req gate for TD-NAME-001 / RF1.4.4 pay-down (baseline 2026-05-14: 245 findings across 9 packages) |
