...
| `lint:no-v1-broker-imports` | [`dev/scripts/lint_no_v1_broker_imports.sh`](../dev/scripts/lint_no_v1_broker_imports.sh) | _(no registry — hard gate)_ | v2 notifications code (`backend/internal/notifications/v2/`) must not import the v1 broker package (`backend/internal/notifications/broker`). Strangler-fig isolation rule — v2 must be standalone until v1 deletion (S16). |
| `lint:no-direct-outbox-write` | [`dev/scripts/lint_no_direct_outbox_write.sh`](../dev/scripts/lint_no_direct_outbox_write.sh) | _(no registry — hard gate)_ | code outside `backend/internal/notifications/v2/` must use `producer.Producer.Enqueue` / `EnqueueTx`, not raw INSERTs into `notifications_events_v2` / `notifications_outbox_v2` / `notifications_event_recipients`. Guarantees idempotency, validation, and audit correctness at the producer boundary. (S02) |
| `lint:no-stub-evaluator` | [`dev/scripts/lint_no_stub_evaluator.sh`](../dev/scripts/lint_no_stub_evaluator.sh) | _(no registry — hard gate)_ | `backend/internal/notifications/v2/rules/evaluator.go` must not contain a stub return (`return true // stub` / `return false // stub`). A stub evaluator matches every event unconditionally — a silent correctness failure. The real evaluator loads rules from the DB and evaluates conditions via the operator/jsonpath engine. (S07) |
