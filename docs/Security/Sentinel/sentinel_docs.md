...
- **Backend:** `backend/internal/sentinel/` — a Go middleware mounted in front of every `/_site/admin/*` and `/samantha/v2/*` route. Reads the JWT, resolves the focus node, calls `topology.Service.ResolveSubtree`, attaches a `sentinel.Clamp` struct to the request context. Downstream handlers MUST read the clamp via `sentinel.FromCtx(ctx)`; bypassing it is lint-banned. The same package also exposes a small writer surface — `PUT /_site/sentinel/focus` — for the per-user home-topology-node preference; the action `sentinel_set_focus(nodeId)` on the frontend is end-to-end durable as of 2026-05-24 (see `sentinel_revision_history.md`).
