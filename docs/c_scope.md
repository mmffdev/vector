...
| Dev ERD page — live + snapshot entity-relationship diagram at `/dev/erd` (PLA-ERD) | DEV / API / SQL / UI | 266 | ERD0–ERD4 | underway (impl done; mig + browser smoke pending) | Backend `backend/internal/erd/` introspects vector_artefacts + mmff_library via `pg_stat_user_tables`, `information_schema.columns`, `pg_constraint`; 60s in-process cache; `GET /_site/admin/dev/erd` returns live JSON, `POST` writes `dev/audits/erd.json` atomically. Frontend `dev/pages/DevErdPanel.tsx` three-column shell with Cytoscape + dagre layout (BT rank), filter rail (DB / group / edge-kind), inspector. Groups sourced from new `dev/audits/erd_groups.yaml` (9 areas seeded; `system_areas.yaml` kept separate — different schema). Mig 266 mirrors 239 grant pattern, 6 system roles. **Pending:** apply mig 266; browser smoke as `gadmin@mmffdev.com`. |
